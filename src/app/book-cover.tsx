...
interface BookCoverProps {
  title: string;
  author: string;
  notBackCover?: boolean;
}

export function BookCover({ title, author, notBackCover = false }: BookCoverProps) {
  return (
    <div className="w-full h-full bg-amber-100 flex justify-center items-center p-4">
      <div className="w-1/2 aspect-[2/3] bg-amber-700 rounded-lg shadow-lg p-8 flex flex-col justify-center items-center relative">
        ...
        <div className={`absolute inset-y-0 w-2 bg-amber-600 rounded-lg shadow-inner ${notBackCover ? 'left-0' : 'right-0'}`}></div>
        ...
        {notBackCover && (
          <p className="mt-8 text-amber-200 text-center">Portafolio</p>
        )}
      </div>
    </div>
  )
